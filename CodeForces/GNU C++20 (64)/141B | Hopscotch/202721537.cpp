...
if(x>=2*a || x<=-2*a ) cout<<-1<<endl;
    ...
    if(res==0){
        ...
        if  (x<g && x>-g )cout<<1<<nl;
        else cout<<-1<<nl;
    }
    else {
        res=y-a;
        res= ceil((double)res/a);
        int ind;
        if(res%2==0){ ind=(res/2)*3; if(x>0) ind++;
          if ( !(x<a && x>-a )  || x==0)ind=-1;}
        else {ind=(res/2)*3+2; double g= (double)a/2;
        if (x>=g || x<=-g) ind=-1;}
cout<<ind<<nl;
