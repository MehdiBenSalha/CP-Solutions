...
int main() {
   ...
   string a,b;
    cin>>a>>b;
    if(a==b) cout<<"YES"<<nl<<a<<nl;
    else {
        if(a[0]==b[0] ) cout<<"YES"<<nl<<a[0]<<"*"<<nl;
        else if(a[a.length()-1]==b[b.length()-1]) cout<<"YES"<<nl<<"*"<<b[b.length()-1]<<nl;
        else{
 
    string l="";
        for (int i = 0; i <a.length(); ++i) {
            for (int j = i; j < a.length(); ++j) {
                string ss=a.substr(i,j-i+1);
 
                if(b.find(ss)!=string::npos){
                    if(ss.length()>l.length())
                        l=ss;
                }
            }
        }
     if(l.length()>1) cout<<"YES"<<nl<<"*"<<l<<"*"<<nl;
       else cout<<"NO"<<nl;
