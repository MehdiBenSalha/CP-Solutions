...
bool validationfunc(int i, int j, int n, int m){
...
ll cntdeep(p32 s){
    ...
    qq.push(s);
    vis[s.first * m + s.second] = true;
    ll deeeeeeeeeeeeeeeeeep = 0;
    while(qq.size()){
        p32 c = qq.front();
        qq.pop();
 
 
        deeeeeeeeeeeeeeeeeep+= a[c.first][c.second];
        // check neighbours
        int dx[] = {-1, 0, 1, 0};
        int dy[] = {0, 1, 0, -1};
        for (int i = 0; i < 4; ++i) {
            ...
            int x = c.first + dx[i];
            int y = c.second + dy[i];
            if(validationfunc(x, y, n, m) && !vis[x * m + y]){
                vis[x*m + y] = true;
                qq.push(make_pair(x, y));
            }
        }}
 
    return deeeeeeeeeeeeeeeeeep;
}
...
void solve(){
    ...
    while(strt.size()){
        p32 cell = strt.back();
        strt.pop_back();
        if( vis[cell.first * m + cell.second]){
            continue;
        }
 
        ll depth = cntdeep(cell);
