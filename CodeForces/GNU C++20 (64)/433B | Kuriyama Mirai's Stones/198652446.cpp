...
sort(v.begin(),v.end());
vector<ll> p1,p2;
        partial_sum(all(tab), back_inserter(p1));
        partial_sum(all(v), back_inserter(p2));
        int q;
        cin>>q;
        while(q--){
            int f,l,r;
            cin>>f>>l>>r;
            if(f==1){
            if(l==1) cout<<p1[r-1]<<endl;
            else cout<<p1[r-1]-p1[l-2]<<endl;}
            else{
                if(l==1) cout<<p2[r-1]<<endl;
                else cout<<p2[r-1]-p2[l-2]<<endl;
            }
        }
