...
pair<double,double> a[n];
        ...
        for(int i=0;i<n;i++){
            double l=a[i].first;
            double r=a[i].second;
            if(x==l) {s.insert({INT_MAX,INT_MAX});continue;}
           double aa= (pos.second - r) / (pos.first - l);
              double b= r - aa * l;
              s.insert({aa, b});
        }
        ...
        for (int i = 0; i < v.size() - 1; ++i) {
            ...
            if(abs(v[i].first-v[i+1].first)<=0.00001 &&  abs(v[i].second-v[i+1].second)<=0.00001) {v.erase(v.begin()+i);i--;}
        }
        cout<<v.size()<<nl;
