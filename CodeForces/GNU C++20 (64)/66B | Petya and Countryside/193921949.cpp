...
bool can(int tab[],int n,int k,int dist) {
int cnt=1;
int sum=0;
int l=0;
    for (int i = 1; i < n; ++i) {
...
int tab[n];
        ...
        for (int i = 0; i < n; ++i) {
            ...
            int m=1;
            for (int j = i+1; j <n-1; ++j) {f= true;
                if(tab[j]>=tab[j+1] && tab[j]<=tab[i]) m++;
                else {m++;f= false;break;}
            }
            if(f) m++;
            f= false;
 
            for (int j = i-1; j >0; --j) {f= true;
                if(tab[j]<=tab[i] && tab[j]>=tab[j-1]) m++;
                else {m++;f= false;break;}
            }
if(f) m++;
