...
bool ordered(int tab[],int tab2[],int n){
    for (int i = 0; i < n; ++i) {
...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < m; ++i) {
            ...
            if (f) columns.push_back(i);
            f = true;
        }
        if ((rows.size() == columns.size() && rows.size() == 0) || (rows.size()==0 && columns.size()!=0) ||  (columns.size() == 0 && rows.size()!=0)) cout << "NO";
        else {
            bool ff = true;
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < m; ++j) {
                    if (a[i][j] == 1 &&( std::find(rows.begin(), rows.end(), i) == rows.end() &&
                        std::find(columns.begin(), columns.end(), j) == columns.end())) {
                        ff = false;
                        break;
                    }
                }
