...
int main() {
    ...
    while (t--) {
        ...
        int l=1,r=m;
        for (int i = 0; i < m-1; ++i) {
            if(tab[i]>tab[i+1]){ l=i+1; break;}
        }
        for (int i = l-1; i <m-1 ; ++i) {
            if(tab[i]<tab[i+1]){ r=i+1; break;}
 
        }
        bool f= true;
        for (int i = r-1; i < m - 1; ++i) {
            if(tab[i]>tab[i+1]) {f= false; break;}
        }
        if(( r!=m && tab[l-1]>tab[r] ) || (l!=1 && tab[r-1]<tab[l-2])) f= false;
        if(m==2 ) f=true;
        if(!f) cout<<"no";
        else {cout<<"yes"<<nl<<l<<" "<<r;}
    }
}