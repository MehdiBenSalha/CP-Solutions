...
int main() {
 ...
 int a0=0,a1=0,b0=0,b1=0;
        ...
        int w;
        int diff;
        if(a1==b1 && ordered(a,b,n)) cout<<0<<nl;
        else{
            if(a1==b1 && !ordered(a,b,n)) cout<<1<<nl;
            else{
 
            int cnt=0;
            if(a1>b1) {w=0; diff=a1-b1;}
            else {w=1; diff=a0-b0;}
 
            for (int i = 0; i <n ; ++i) {
                if(a[i]!=w && b[i]==w) {diff--; cnt++; a[i]=w;}
                if(diff==0) break;
            }
            for (int i = 0; i < n; ++i) {
                if(a[i]!=b[i] && diff!=0){diff--;cnt++; a[i]=b[i];}
                if(diff==0) break;
            }
            for (int i = 0; i < n; ++i) {
