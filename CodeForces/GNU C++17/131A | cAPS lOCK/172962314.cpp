...
string s;
cin>>s;
if(s.length()==1) { char j=s[0];
    if(int(j)>90) cout<<(char)toupper(j);
else cout<<(char) tolower(j);}
else {
string ss=s,sss=s;
    std::transform(ss.begin(), ss.end(),ss.begin(), ::toupper);
 
    std::transform(s.begin()+1, s.end(),s.begin()+1, ::toupper);
    if( sss==s && (int)sss[0]>91) {string m=s;
        std::transform(m.begin(), m.end(), m.begin(), ::tolower);
        m[0]-=32;
 
        cout<<m;
    }
    else {if(sss==ss) {string m=s;
        std::transform(m.begin(), m.end(), m.begin(), ::tolower);
    cout<<m;}
    else
cout<<sss;}}
