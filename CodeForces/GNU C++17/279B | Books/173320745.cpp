...
bool can(ll tab[], int n , int k,int s){
 
    for (int i = 0; i <=n-s; ++i) {
        if(i==0){int x=tab[s-1]; if(x<=k) return true;}
        else {int x=tab[i+s-1]-tab[i-1]; if(x<=k) return true;}
    }
    return false;
}
//ll f[1000001]={0};
...
ll pre[n];
        fillPrefixSum(tab,n,pre);
       int l=0,r=n;
        int ans=0;
        while (l<=r){
            int mid=(l+r)/2;
            if(can(pre,n,k,mid)){l=mid+1; ans=mid;}
            else r=mid-1;
        }
        cout<<ans;
